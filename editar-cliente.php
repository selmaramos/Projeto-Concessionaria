...
<?php
    ...
    if (!isset($_REQUEST['id_cliente']) || empty($_REQUEST['id_cliente'])) {
        print "<script>alert('Acesso inválido. ID do cliente não fornecido.');</script>";
    ...
    if ($res->num_rows === 0) {
        print "<script>alert('Cliente não encontrado no banco de dados. Retornando para lista.');</script>";
        print "<script>location.href='?page=listar-cliente';</script>";
        exit;
    }
